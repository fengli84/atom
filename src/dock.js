...
module.exports = class Dock {
  ...
  pointWithinHoverArea (point, includeButtonWidth) {
    ...
    // The are used when detecting "leave" events is actually larger than when detecting entrances.
  ...
  deserialize (serialized, deserializerManager) {
    ...
    this.setState({
      ...
      // If no items could be deserialized, we don't wan to show the dock (even if it was oepn last time)
